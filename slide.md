...
これをElasticsearchだったり、PostgreSQLのデータベースだったりに格納したいとします。

\vspace{1em}
すると、誰かが当然このアクセスログをパースして、  
JSONだったりSQLだったりに直して送る必要がある。
...
\vspace{1em}
ElasticsearchとPostgreSQLを例に出しましたが、Kafkaだったり、S3だったりにも送れます。  
...
Ruby 3.2に移行すると何がいいかというと、YJITが使えるんですが、  
これは素晴らしいの一言につきる。  
